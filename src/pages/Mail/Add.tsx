...
import {Row, Radio, Button, Form, Input, Space, Card, message, Upload} from "antd";
...
const Add = (props: any) => {
  ...
  useEffect(() => {
    if (id) {
      dispatch({
        type: 'mail/getEmailInfo',
        payload: {id},
      }).then((res: any) => {
        if (res.msg === 'success' || res.code === 0) {
          const {type, edition, title} = res.Email

          if (res.Email.enclosure) {
            const r = res.Email.enclosure.split(',').map((item: any, index: any) => {
              return {
                uid: index + 1,
          ...
          form.setFieldsValue(res.Email)
          form.setFieldsValue({
            type,
            edition,
            title,
            content: BraftEditor.createEditorState(res.Email.content),
          })
        }
        if (res.code === 500) {
    ...
    <PageContainer>
      <Card>
        <Form labelCol={{span: 3}} onFinish={onFinish} form={form}>
          <Form.Item label="业务类型" name="type" rules={[{required: true, message: '请输入客户名称'}]}>
          ...
          <Form.Item label="语言版本" name="edition" rules={[{required: true, message: '请输入英文名称'}]}>
            <Radio.Group onChange={() => {
            }} value={1}>
